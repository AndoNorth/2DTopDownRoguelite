...
public static class GeneralUtility
{
    ...
    public class World_Bar
    {
        ...
        public World_Bar(Transform parent, Vector3 localPosition, Vector3 localScale, Color? backgroundColor, Color barColor, float sizeRatio, int sortingOrder, string sortingLayer, Outline outline = null)
        {
            SetupParent(parent, localPosition);
            if (outline != null) SetupOutline(outline, localScale, sortingOrder - 1, sortingLayer);
            if (backgroundColor != null) SetupBackground((Color)backgroundColor, localScale, sortingOrder, sortingLayer);
            SetupBar(barColor, localScale, sortingOrder + 1, sortingLayer);
            SetSize(sizeRatio);
        }
        ...
        private void SetupOutline(Outline outline, Vector3 localScale, int sortingOrder, string sortingLayer)
        {
            CreateWorldSprite(transform, "Outline", GameAssets.instance.white1x1, new Vector3(0, 0), localScale + new Vector3(outline.size, outline.size), sortingOrder, sortingLayer, outline.color);
        }
        private void SetupBackground(Color backgroundColor, Vector3 localScale, int sortingOrder, string sortingLayer)
        {
            background = CreateWorldSprite(transform, "Background", GameAssets.instance.white1x1, new Vector3(0, 0), localScale, sortingOrder, sortingLayer, backgroundColor).transform;
        }
        private void SetupBar(Color barColor, Vector3 localScale, int sortingOrder, string sortingLayer)
        {
            ...
            Transform barIn = CreateWorldSprite(bar, "BarIn", GameAssets.instance.white1x1, new Vector3(localScale.x * 0.5f, 0), localScale, sortingOrder, sortingLayer, barColor).transform;
        }
    ...
    public static GameObject CreateWorldSprite(string name, Sprite sprite, Vector3 position, Vector3 localScale, int sortingOrder, string sortingLayer, Color color)
    {
        return CreateWorldSprite(null, name, sprite, position, localScale, sortingOrder, sortingLayer, color);
    }

    // Create a Sprite in the World
    public static GameObject CreateWorldSprite(Transform parent, string name, Sprite sprite, Vector3 localPosition, Vector3 localScale, int sortingOrder, string sortingLayer, Color color)
    {
    ...
    public class UI_Bar
    {
        ...
        {
            return DrawSprite(outline.color, gameObject.transform, Vector2.zero, size + new Vector2(outline.size, outline.size), "Outline");
        }
        private void SetupBackground(Color backgroundColor)
        {
            background = DrawSprite(backgroundColor, gameObject.transform, Vector2.zero, Vector2.zero, "Background");
            background.anchorMin = new Vector2(0, 0);
            background.anchorMax = new Vector2(1, 1);
        }
        private void SetupBar(Color barColor)
        {
            bar = DrawSprite(barColor, gameObject.transform, Vector2.zero, Vector2.zero, "Bar");
